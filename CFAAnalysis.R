...
CFA.data[which(CFA.data$gender==1),]$gender <- "Female" 
CFA.data[which(CFA.data$gender==0),]$gender <- "Male" 




# Effect of bilingualism on these factors
lang.lm.1 <- lm(language~bilec_total_input*diagnosis + age_m*wasi_sum_rawscores + gender, data=CFA.data)
lang.lm.2 <- lm(language~bilec_total_input*diagnosis + age_m+wasi_sum_rawscores + gender, data=CFA.data)
lang.lm.3 <- lm(language~bilec_total_input*diagnosis*age_m+wasi_sum_rawscores + gender, data=CFA.data)
AIC(lang.lm.1); AIC(lang.lm.2); AIC(lang.lm.3) # Non-interation term better
summary(lang.lm.2)

soc.cog.lm.1 <- lm(social.cognition~bilec_total_input*diagnosis + age_m*wasi_sum_rawscores+ gender, data=CFA.data)
soc.cog.lm.2 <- lm(social.cognition~bilec_total_input*diagnosis + age_m+wasi_sum_rawscores+ gender, data=CFA.data)
soc.cog.lm.3 <- lm(social.cognition~bilec_total_input*diagnosis*age_m+wasi_sum_rawscores+ gender, data=CFA.data)
AIC(soc.cog.lm.1); AIC(soc.cog.lm.2); AIC(soc.cog.lm.3) # Non-interation term better
summary(soc.cog.lm.2)

exec.func.lm.1 <- lm(-executive.function~bilec_total_input*diagnosis + age_m*wasi_sum_rawscores+ gender, data=CFA.data)
exec.func.lm.2 <- lm(-executive.function~bilec_total_input*diagnosis + age_m + wasi_sum_rawscores+ gender, data=CFA.data)
exec.func.lm.3 <- lm(-executive.function~bilec_total_input*diagnosis*age_m + wasi_sum_rawscores+ gender, data=CFA.data)
